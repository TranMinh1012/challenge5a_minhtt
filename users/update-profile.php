<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "UPDATE users SET phone = '{$phone}', email = '{$email}', password = '{$password}'";
        if (move_uploaded_file($tmp_name, str_replace('\\', '/', dirname(__FILE__, 2)) . '/assets/avatars/' . $img_name)) {
            $sql .= ", avatar = '{$img_name}'";
        }
        if ($_SESSION['role'] == 0) {
            $sql .= ", name = '{$name}', username = '{$username}'";
        }
        $sql .= "  WHERE id = {$id}";
    ...
    <div class="col-lg-12">
        <form action="" method="POST" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="Nhập tài khoản" id="username" name="username" value="<?= $user['username'] ?>" <?= $_SESSION['role'] == 0 ? 'required' : 'disabled' ?>>
            </div>
            <div class="form-group">
                <label for="name">Họ tên: <span class="text-danger">*</span></label>
                <input type="text" class="form-control" placeholder="Nhập họ tên" id="name" name="name" value="<?= $user['name'] ?>" <?= $_SESSION['role'] == 0 ? 'required' : 'disabled' ?>>
            </div>
            <div class="form-group">
